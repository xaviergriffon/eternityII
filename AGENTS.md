...
| `src/app/`  | Entry point, client/server roles, signals, globals, GPU | `main`(c) `etii_client` `etii_server` `etii_control` `control_registry` `known_clients_registry` `client_config` `server_config` `fork_gate` `fork_orchestrator` `work_broker` `app_runtime` `etii_statistic`(h) `app_static_variables` `gpu_pruner`(.cu/.h) |
...
| `src/app/work_broker.c` | `--local-dispatch`: parent relays its forks' delegated possibilities to the server, off the search thread's hot path |
