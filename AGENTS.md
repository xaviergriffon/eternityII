...
**Cross-document links are checked by `make check-doc-links`** (`tools/check_doc_links.py`, deliberately not wired into `make test` — it's a tool, like `gen-root`). It replays GitHub's anchor algorithm and reports « ancre absente » separately from « fichier absent », because they are repaired differently. **A stale `#anchor` is repaired by repointing the link, never by renaming the heading** — headings are quoted verbatim from other documents and from AGENTS.md, so shortening one to fit an old link just moves the breakage. GitHub shows no error for an unknown anchor (it opens the top of the file), which is why 34 dead cross-references accumulated unnoticed; run the target after touching any heading. Detail: [docs/tests_et_ci.md](docs/tests_et_ci.md#garde-fou-des-renvois-de-documentation-make-check-doc-links).
...
Other top-level dirs: `data/` (puzzle definitions), `build/` (compilation objects, mirrors `src/`, gitignored), `tests/` (unit tests mirroring `src/`, plus `tests/bench/` and `tests/tools/` for standalone benches and tools — each with its own `.PHONY` Makefile target, never wired into `make test`, while their pure cores are). **Adding a `.c` means dropping it under the right `src/<domain>/` and adding its `build/<domain>/<name>.o` to the `OBJS` list — and the source to `PROD_SRCS` (or `TEST_MODULES`/`TEST_SUITES_COMMON` for a test) in `CMakeLists.txt`.** The two build systems describe the same binaries and only the makefile is exercised by CI, so their lists drifted until `run_tests_16` no longer linked; `tests/tools/check_build_lists.py` now compares them — `make check-build-lists` (a dependency of `make test`) and the CTest entry `build-lists-sync` — and fails naming each gap. Full diagram and module-by-module responsibility table: [docs/architecture.md](docs/architecture.md).
...
make check-build-lists        # Guard: CMakeLists.txt's source lists match the makefile's (run by `make test`)
...
- **`alloc` and `b_faceused` are REBUILT at decode, never stored** — fully derivable from the grid (0 divergence over 3,4 M real packets). Storing `alloc` when it contradicts the grid was possible for zero bytes and was rejected: the eleven production writes of `alloc` are, bar one, literally `possibility_placed_count(...)`, `import()` recomputes it unconditionally on every restored possibility, and AGENTS.md defines it as the count of non-empty cells — storing it would have preserved a value the codebase already repairs everywhere else. **Consequence, accepted deliberately: the pool holds a CANONICAL form.** A packet whose `alloc`/`b_faceused` contradicts its grid does not come back identical, and therefore no longer matches in the analysed pool's index (its dedup compares x, y, alloc, the used pieces and the board). Production never produces one; test fixtures did, and were made coherent (`tests/packet_fixture.h`) rather than the format made lossless. The one production path where an operator can declare a contradictory `alloc` is `read_from_json` (`loadJson`), now normalised on insertion exactly as `import()` normalises a `.back`.
...
**Key invariants**:
- **A refused ADD has TWO unrelated causes, and they must not be conflated** (`DATAMANAGER_ADD_REFUSED_RAM_CAP` vs `_POOL_LOCKED`, `core/datamanager.h`): the RAM cap (only possible under `--stock-max-ram`, recourse = spillover or a higher cap), and every stock file staying locked past `DATAMANAGER_TRYLOCK_MAX_SWEEPS` — a maintenance (consistent backup, sort, restore, descendant purge), which happens **with no cap at all** and resolves by itself. Both return non-zero and mean "nothing inserted, safe to retry", which is why one return value used to serve for both: `add_possibility_waiting_for_room` then blamed the cap unconditionally, and an `expand` under an UNLIMITED cap logged « plafond RAM atteint […] relever --stock-max-ram ou vérifier --stock-spill-dir » while waiting on an autobackup that cleared after 28 s. The reason is re-read on every retry (a wait can start on one cause and continue on the other), and the relief hook is only called for the cap — the spill cannot unlock a mutex. **Only the cap suspends an expansion pass** (`expand_note_wait`): suspending on a maintenance protects nothing — the rest of the pass is reinjected through the same waiting path, so it waits just as long — and burns one of `--expand-max-levels`, a budget of PASSES. Measured on a production stock: the same pass across the same backup went from 12 333 491 possibilities « réinjectées telles quelles » to 13 291 686 carried to completion.
- **Nothing decides on `datamanager_ram_limit_packets()`.** It converts the cap into a possibility COUNT for display only, at the *observed* record size; the criterion is always bytes (`datamanager_resident_bytes` vs `datamanager_ram_limit_bytes`, via `datamanager_has_ram_headroom` / `datamanager_ram_pressure_at_least`). Two sites did decide on it, at the *raw packet* tariff, and both fired at roughly one fifth of the real cap: the between-passes wait of `expand_datas_to_level` and the RAM-pressure detection of `--auto-roles`. The same stale tariff also made the console announce « plafond 1024 Mo (~1 698 958 possibilités) » to a server already holding 3 407 891 in 395 Mo.
...
- **The maintenance window NESTS — `maintenance` (`core/datamanager.c`) is a depth counter, never a flag.** Only `maintenance_enter`/`maintenance_leave` may touch it (`lock_all_file`, `lock_all_file_analysed`, `consistent_backup` and `datamanager_begin_maintenance` all go through them); the decrement saturates at 0, because a negative depth would read as a *permanent* maintenance and silently condemn every later backup to `BACKUP_SKIPPED_MAINTENANCE`. As a flag it broke its only external promise: `restore_apply` (`ui/command_lines.c`) opens a window meant to cover the whole restore sequence, but `restore()`'s own `unlock_all_file()` reset it to 0 *before* the import, so the spill thread went back to evicting mid-replacement. Access is via `__atomic_*` — these sites are cold (a handful of calls per backup/restore, against the millions of packets they move), and unlike a flag a counter never forgives a lost update. **Holding the window obliges the holder**: `stock_spill_step` is inert throughout it, so a holder that needs RAM must free it itself through `datamanager_set_ram_relief_hook`, never by waiting for the spill thread's tick. `restore_keeps_the_maintenance_window_open_through_the_import` and `import_makes_room_itself_when_it_holds_the_maintenance_window` (`tests/core/test_stock_spill.c`) lock the two halves of that bargain, both in a fork with `alarm()` so a regression fails instead of hanging CI.
...
make check-doc-links     # Guard: internal Markdown links — target file exists AND #anchor still matches a heading
```
...
**`make test` never writes into the repository.** `tests/test_main.c` enters a `mkdtemp` sandbox (`test_sandbox_enter`, `tests/sandbox.{h,c}`) before the first suite, so every relative write the production code does under test — `./eternityII.back`, `events.log`, `solution_*`, `*.spillcount`, the `etii_*` Unix sockets — lands there instead of the repo root. This exists because a 1,96 Go production stock at the repo root was *overwritten by a `make test`* (the 4×4 build's `consistent_backup("./eternityII.back", …)`), invisible to the VCS since those names are all ignored. Consequences when writing a test: **writing under a relative path is safe**; **reading repo data by a relative path is not** — the two production globals that do (`parts_files`, `indices_file`) are made absolute *before* the `chdir`, anything else must be built by the test itself; a test that `chdir`s must come back. The guard is `sandbox_suite` (`tests/test_sandbox.c`), run **last**: it compares the launch directory against a snapshot taken at startup and fails naming the file that appeared — which is what catches the two leaks the sandbox cannot (an absolute-path write, a stray `chdir`). Full account, and the sabotage that validates it: [docs/tests_et_ci.md](docs/tests_et_ci.md#le-runner-travaille-dans-un-bac-à-sable-jamais-dans-le-dépôt).
...
**Four caches are maintained incrementally alongside the board and must stay in lockstep with it** — `constraints[][]` (neighbour colours, `bt_propagate_place`/`_undo`), `used[]` (64-bit mirror of the used-piece mask, `mrv_used_set`/`_clear`), `bt_frontier` (the `empty`/`constrained` bitmasks `mrv_choose_cell` enumerates, `bt_frontier_place`/`_undo`), and `cell_mask[]` (the resolved `bucket_id_mask` pointer per cell, `bt_mask_init`/`bt_mask_refresh` — it removes 54,5 index resolutions per node from the MRV scan, measured ×1,22).

**Since §4.16 the learned failure weight is the PRINCIPAL criterion, not a third-rank tie-break** — it *divides* the MRV score and therefore outranks `nconstr`: `score = count << mrv_wdeg_shl(weight)`, the shift falling from `MRV_WDEG_SHIFT` (4) to 0, one less per *doubling* of the weight (a ×16 cap). A **shift, never a division**: the scan is issue-bound, and even a reciprocal table would cost an extra `imul`; `bt_frontier.wshl` is one byte per cell, a fifth lockstep field written wherever `weight` is. The composite key shrinks accordingly — the weight leaves the low fields, `nc_key` drops to 16 bits and is just `(MRV_KEY_NC_MAX - nconstr) << MRV_POS_BITS | pos` — and since the winning key no longer contains `count`, that count is recomputed for the winning cell alone (four popcounts per node, outside the hot loop). **Load-bearing invariant: a shift is a multiplication by a power of two, so `score == 0 ⟺ count == 0`** and the fast path's dead-subtree verdict (`(best >> MRV_KEY_LOW_BITS) == 0`) is unchanged — do not replace the shift by any form that can map a non-empty domain to score 0. Measured over three disjoint depth regimes of a production stock at a 2M-node cap: −96 % of paired refutation nodes and 497 → 500 closures at ≥130 placed pieces, **124 → 287 closures out of 300 between 100 and 120 pieces** (the regime §4.14 declared inconclusive), drop point down 4,5 to 25 points, 16,3 s → 0,40 s end to end, and on the find side 29/1 paired on 30 clones. Own cost of the mechanism, isolated on a node-for-node identical tree: +1,4 %. Two rejected arms are documented there rather than re-tried: weighting the dead cell only (+46 %) and last-conflict reasoning (+4,4 % and −27 % throughput). **Every site that writes `board.grid[cx][cy]` must update all four**, and the `bt_frontier_*` and `bt_mask_refresh` calls sit *next to* the `bt_propagate_*` call rather than inside it, so a new placement site can forget them — `DEBUG_CHECK_POSSIBILITY` builds re-derive `used`, `bt_frontier` and `cell_mask` from the board at every node and log a loud desync precisely to catch that. `bt_mask_cache_stays_in_lockstep_with_constraints` (`tests/core/test_etii_search.c`) locks the fourth one directly, and carries a counter-check — a placement without refresh must make the cache diverge — so it cannot pass if `bt_mask_refresh` ever became a no-op. `bt_frontier.nconstr` (constrained sides per cell) is not bookkeeping for the bitmasks alone: **it is also MRV's tie-break**, and its direction is measured, not derived. At equal MRV score the cell with the MOST constrained sides wins — the *opposite* of the textbook CSP degree heuristic, which loses by +4,4 % of refutation cost where this one gains −6,3 % (§4.12 of [docs/conception/elagage_recherche.md](docs/conception/elagage_recherche.md)). Flip that comparison and two tests fail — and the tie-break now lives inside a **composite key** (`key = (count << (NC + W)) | ((MRV_KEY_NC_MAX - nconstr) << W) | (MRV_KEY_W_MAX - weight)`) whose natural ordering *is* the rule, so the whole choice reduces to one branchless comparison; the field widths are locked by compile-time checks (`mrv_key_nc_fits_check`, `mrv_pos_fits_check`) because a low field overflowing into `count` would silently corrupt the MRV criterion itself. **The composite key's remaining low field is the learned failure weight's former home** (`bt_frontier.weight`, §4.14): `bt_forward_check`/`_fast` call `bt_fc_failed` on every refusal, bumping the dead neighbour AND the placed cell (`bt_frontier_fail`, saturating, global halving at 255, reset per root by `bt_frontier_init`). As a third-rank tie-break it was worth −79,6 % of refutation nodes; the opposite direction costs +210 %, and even a random tie-break beats the positional order by 37–56 %. §4.16 then promoted it to the principal criterion (see below) — the direction is unchanged and still measured, only its rank moved. Do not flip it, do not revert to positional order without re-measuring. The frontier's bit order (`pos = x * ETERN_SIZE + y`) remains load-bearing where the weight also ties: it reproduces the `for x { for y }` order of the scan it replaced. `tests/core/test_etii_search.c` locks both with an oracle comparing `mrv_choose_cell` against an independent full scan — one that derives the constrained-side count from `constraints[][]` rather than from `nconstr`, so the oracle checks `nconstr` itself too. **The engine has two code paths, chosen once per search, never per cell** (`bt_masks_complete`, `src/core/etii_search.c`): on a production map (compact index + `bucket_id_mask` with exactly `BT_MASK_WORDS` words) it takes `mrv_choose_cell_fast`/`bt_forward_check_fast`, compiled with a constant mask width and no NULL test — `bt_mask_init`/`bt_mask_refresh` substitute the shared `bt_zero_mask` for an empty bucket's NULL in that mode, while `map_bucket_id_mask`'s own NULL-for-empty contract is untouched; hand-built test maps and out-of-template maps stay on the generic, traversal-fallback versions. Both paths must give identical results — `mrv_choose_cell_fast_matches_generic_on_real_map` and `bt_forward_check_fast_same_verdict_as_generic` lock that, with the fixture's max id forced to `ETERN_PARTS` so the map is « complete » under every compiled puzzle size. `bt_frontier.nc_key` (`((7 - nconstr) << 16) | ((255 - weight) << 8) | pos`, the low part of the fast scan's combined choice value) is a fifth lockstep field, written at the same three sites as `nconstr` (`bt_frontier_init`/`_constrain`/`_release`) plus `bt_frontier_fail`. **The fast scan is issue-bound (IPC ≈ 3,15): each µop per frontier cell costs ~0,25 cycle × 54 cells per node, and two « memory » hypotheses (inline masks, save/restore on undo) were measured and lost** — see the écarté table in [docs/autosearch_step.md](docs/autosearch_step.md) §1.3 quater before re-proposing them. **The hot loop's pruning counters (`fc_attempts`, `fc_pruned`, `fc_cells_studied`, `fc_pruned_at[]`) are written with a relaxed load + relaxed store (`fc_stat_bump`), never `__atomic_fetch_add`**: in any one process a single thread writes them (`run_mono_client` runs the search in the current thread; `NB_THREADS` counts *forks*, i.e. processes, each with its own copy aggregated later over IPC), so the locked read-modify-write was a lock taken against nobody — −5,0 % of cycles. This is not a data race: both accesses stay atomic, only whole-RMW atomicity goes, which a single writer does not need — the same contract `counters[]` has always had. **Cold paths keep `__atomic_fetch_add`** (`bt_materialize_pending`, and `possibility.c` which the console thread can reach through `removeNoNext`). Freshness is unchanged by design, and that is load-bearing: `bench_poll_and_maybe_stop` reads these counters *while* the search runs, and the bench relies on `fc_attempts = nodes + fc_pruned` holding to within 1. A deferred-publication variant (PR #260) breaks that identity by ~199 and was rejected for it — `fc_counters_are_visible_as_soon_as_the_forward_check_returns` guards against reintroducing it.
...
**The find-side campaign is closed, and its result is negative — don't reopen it without reading §7 first.** 2 640 runs over four instance regimes: no value order beats production's arena order (`rare_first`, §4.8) — the five orders tie on *nodes* everywhere and all lose on *time* (1,12× to 1,42× per node), `lcv` (the textbook CSP value heuristic) losing on nodes too in the hard regimes. Three method rules the campaign paid for: **measure time, not just nodes** (`mcv` wins on nodes in one cell and would have been adopted on that alone); **count undecided pairs as undecided** (on hard cells it is the solved-fraction, 14/60 vs 54/60, that rejects `center`, not the pairing); and **a censored baseline concludes nothing** (the no-restart cost is only a lower bound once a run hits the cap, and the censoring favours restarting). **"Value order is neutral for refutation" holds only while the VARIABLE order is independent of the VALUE order** — a tie-break that learns from the search couples them, and `bench_solve`'s self-test detects and reports that instead of blaming the permutation. Also settled there: the annexe's solution-counting model orders difficulty correctly *within* a size and **inverts across sizes**, so it is not a hardness scale.
