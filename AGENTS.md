...
**The hot loop's pruning counters are accumulated locally, never written atomically per attempt.** `bt_forward_check` writes into an `fc_local_stats` supplied by its caller and **never touches the `fc_*` globals itself**; `search_packet_backtracking_mrv` publishes every `FC_STATS_PUBLISH_INTERVAL_NODES` nodes and at every exit (including before `record_solution`, which may `exit()`). Totals stay exact and monotone — publishing adds, never overwrites — but a reader that samples *while the search runs* (`bench_poll_and_maybe_stop`) can miss up to one window, so `fc_attempts ≈ nodes + fc_pruned` is no longer exact to 1. The window size is set by that reader's precision, not by amortization (which saturates far earlier): see the constant's comment before changing it, and `docs/tests_et_ci.md` for the resulting ±0,0005 point on the reported prune rate.

**Three caches are maintained incrementally alongside the board and must stay in lockstep with it** — `constraints[][]` (neighbour colours, `bt_propagate_place`/`_undo`), `used[]` (64-bit mirror of the used-piece mask, `mrv_used_set`/`_clear`), and `bt_frontier` (the `empty`/`constrained` bitmasks `mrv_choose_cell` enumerates, `bt_frontier_place`/`_undo`). **Every site that writes `board.grid[cx][cy]` must update all three**, and the `bt_frontier_*` call sits *next to* the `bt_propagate_*` call rather than inside it, so a new placement site can forget it — `DEBUG_CHECK_POSSIBILITY` builds re-derive `used` and `bt_frontier` from the board at every node and log a loud desync precisely to catch that. The frontier's bit order (`pos = x * ETERN_SIZE + y`) is load-bearing, not cosmetic: it reproduces the `for x { for y }` order of the scan it replaced, which is what keeps MRV's tie-breaking — and therefore the explored tree — unchanged. `tests/core/test_etii_search.c` locks this with an oracle test comparing `mrv_choose_cell` against a verbatim copy of the old 256-cell scan.
