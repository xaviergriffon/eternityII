...
**Key invariants**:
- **A refused ADD has TWO unrelated causes, and they must not be conflated** (`DATAMANAGER_ADD_REFUSED_RAM_CAP` vs `_POOL_LOCKED`, `core/datamanager.h`): the RAM cap (only possible under `--stock-max-ram`, recourse = spillover or a higher cap), and every stock file staying locked past `DATAMANAGER_TRYLOCK_MAX_SWEEPS` — a maintenance (consistent backup, sort, restore, descendant purge), which happens **with no cap at all** and resolves by itself. Both return non-zero and mean "nothing inserted, safe to retry", which is why one return value used to serve for both: `add_possibility_waiting_for_room` then blamed the cap unconditionally, and an `expand` under an UNLIMITED cap logged « plafond RAM atteint […] relever --stock-max-ram ou vérifier --stock-spill-dir » while waiting on an autobackup that cleared after 28 s. The reason is re-read on every retry (an wait can start on one cause and continue on the other), and the relief hook is only called for the cap — the spill cannot unlock a mutex.
