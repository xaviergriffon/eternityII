...
### Fixed traversal order (default engine)

`directions[]`/`dirx[]`/`diry[]` (`src/app/static_variables.c`, `ETERN_PARTS == 256` block) fix the cell-filling order `search_packet_backtracking_core` uses. The order shipped since **v13** of the protocol comes from a **static greedy risk-score search**: at each step, pose the cell whose remaining-candidate pool is smallest (corner = 4, edge = 56, interior = 196, the pool shrinking globally as the traversal proceeds), the risk score being position weighted by 1/remaining-pool-size at the moment of placement — rarer cells posed early waste less search before a possible dead end. Measured **−1.3 %** risk score versus the prior (edge/interior interleaved) order, comparable to the best other candidate tried (immediate corners, −1.4 %) — reproducible analysis and full method: [docs/conception/elagage_recherche.md](docs/conception/elagage_recherche.md) §3.2. Structurally: the 5 official index cells (pieces 139/208/255/181/249) come first (non-branching, always skipped), then the full 60-cell frame in one contiguous pass, then a systematic interior sweep — no more interleaving between frame and interior. **Changing this order changes what `alloc` means for the same cursor index** — exactly what forced the v11 bump; this reorder forced the same bump again (v13): [docs/echanges_client_serveur.md](docs/echanges_client_serveur.md#canal-de-contrôle-v9).
