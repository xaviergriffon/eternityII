...
- `expand_datas_to_level` **and `import`** never drop a possibility on a RAM-cap refusal — they **wait**, bounded only by `REQUEST_STOP`, never by a fixed timeout: a stuck configuration should stall visibly (logged every 5s), not lose data silently. **`put_to_pool` refuses for TWO unrelated reasons behind the same return code 1** — the `--stock-max-ram` cap, and its trylock budget running out while the files stay locked (backup/maintenance) — so any path that *logs* a refusal must ask for the reason (`put_to_local_with_reason`, `datamanager_add_refusal_t`). Conflating them made a server running on **unlimited RAM** write `expansion : plafond RAM atteint pendant cette passe` into `events.log` during a backup: a false diagnosis pointing at an option that had no bearing on it. The between-passes wait (`expand_wait_for_ram_headroom_between_passes`) is gated on the RAM reason alone — a momentarily locked stock frees itself, there is no room to wait for. `put_to_pool` refuses *without inserting anything*, so ignoring its return value is a silent loss — that was a real bug in `import` (3 407 891 possibilities restored under `--stock-max-ram 1024` kept 1 756 302, **lost 1 651 589**). The wait **makes room itself** through `datamanager_set_ram_relief_hook` (injected by `app/`, since `core/` must not depend on `core/stock_spill.c`): waiting on the spill thread's own tick — 4096 packets per 100 ms — is far below a bulk import's rate. That hook is `stock_spill_relieve`, not `stock_spill_step`: the latter deliberately no-ops under a maintenance window, which would turn the wait into a deadlock for a caller that *holds* that window.
