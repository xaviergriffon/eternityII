...
Other top-level dirs: `data/` (puzzle definitions), `build/` (compilation objects, mirrors `src/`, gitignored), `tests/` (unit tests mirroring `src/`, plus `tests/bench/` and `tests/tools/` for standalone benches and tools — each with its own `.PHONY` Makefile target, never wired into `make test`, while their pure cores are). **Adding a `.c` means dropping it under the right `src/<domain>/` and adding its `build/<domain>/<name>.o` to the `OBJS` list (and to `add_executable` in `CMakeLists.txt`).** Full diagram and module-by-module responsibility table: [docs/architecture.md](docs/architecture.md).
