...
struct client_statistics {
    ...
    /** Profondeur de la racine actuellement reçue du serveur (`lastroot`), -1 = inconnu/idle. */
    int root_depth;
    /** Profondeur minimale encore en attente dans la pile de ce fork (`lastdepth`,
        `bt_min_pending_depth`) -- PAS la profondeur du chemin courant. -1 = inconnu/idle. */
    int min_pending_depth;
