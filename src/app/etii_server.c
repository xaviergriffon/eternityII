...
void runserver(const char* file)
{
    ...
    if (HTTP_PORT > 0 && HTTP_ADMIN_TOKEN[0] == '\0') {
        ...
        // maxStockByThread, shallowRootAbandonDepth, prunerBatch,
        // clientsCommand/clientsCmd, restore, backup) exige désormais un jeton Bearer valide (cf.
        ...
        // fonctionnelle.
        log_info("API HTTP admin démarrée sans --http-token-file : les commandes de modification (pause, resume, limit, maxStockByThread, shallowRootAbandonDepth, prunerBatch, clientsCommand, restore, backup) resteront inaccessibles via POST /api/v1/command (401)\n");
    }
