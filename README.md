...
- `--local-dispatch` — client : le process parent relaie au serveur les délégations de ses forks, au lieu que chaque fork paie un aller-retour TCP synchrone **par possibilité** depuis son thread de recherche. Désactivée par défaut, **recommandée pour un client de recherche** (×2,8 de travail exploré à durée égale, mesuré). Sans effet sur les forks pruner. Voir [Utilisation](docs/utilisation.md#relais-des-délégations-par-le-parent---local-dispatch).
- `--split-shallow-first` — client : cède les frères les moins profonds d'abord au lieu des plus profonds. Désactivée par défaut, **piste non tranchée** (la première mesure ne montre pas de gain). Voir [Utilisation](docs/utilisation.md#sens-de-cession-des-frères-non-explorés---split-shallow-first).
