...
- `--stock-spill-dir <chemin>` — déporte sur disque plutôt que refuser une fois le plafond RAM approché (défaut `./eternityii-spill`, survit à un `backup` suivi d'un `restore` ; la file de travail d'une expansion compte dans le plafond, rien n'est rechargé depuis le disque pendant une expansion, et une passe développe aussi la part du stock déportée sur disque, lue par le bas de la pile). Voir [Utilisation](docs/utilisation.md#débordement-sur-disque-du-stock---stock-spill-dir).
