...
> Détails (scripts d'intégration, Docker, couverture, matrice CI, banc de mesure `ETII_BENCH_NODES`, outil `make gen-root` qui convertit un plateau externe en racine de stock, outil `make border-mass` qui mesure la masse totale des anneaux de bordure — et, avec `--save-rings FILE --max-rings N` sans `--dp`, en échantillonne des anneaux réels comme racines `.back` — ~190 k/s en régime soutenu, le disque étant le facteur limitant, ou en format compact `--rings-format packed6`, 12,8× plus petit et toujours indexable ; `make check-rings` vérifie le fichier produit — cohérence et absence de doublon — en une passe à mémoire bornée) : [docs/tests_et_ci.md](docs/tests_et_ci.md) — conventions d'écriture des tests : [tests/README.md](tests/README.md).
