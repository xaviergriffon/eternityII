...
- `--stock-hot-floor <pct>` / `--stock-hot-reload <pct>` — sous `--stock-max-ram`, le stock froid est rangé dans un **étage RAM en blocs** (≈ 70 octets par possibilité contre 112 en liste chaînée, **31 compressé par zstd** avec `make ZSTD=1` ; avec ou sans disque) ; la liste chaude y descend au-dessus de son plancher (défaut 25 % du plafond) et en remonte sous son seuil de rechargement (défaut 10 %). Sauvegardé dans le `.back`, remplacé par `restore`, lu par l'expansion. Voir [Utilisation](docs/utilisation.md#étage-ram-en-blocs---stock-hot-floor---stock-hot-reload).
- `--stock-spill-dir <chemin>` — déporte sur disque plutôt que refuser une fois le plafond RAM approché (défaut `./eternityii-spill`, survit à un `backup` suivi d'un `restore` — `backup` recopie le débordement DANS `eternityII.back`, un seul fichier restaurable ailleurs ou sous un plafond plus bas ; la file de travail d'une expansion compte dans le plafond, rien n'est rechargé depuis le disque pendant une expansion, et une passe développe aussi la part du stock déportée sur disque, lue par le bas de la pile). Voir [Utilisation](docs/utilisation.md#débordement-sur-disque-du-stock---stock-spill-dir).
