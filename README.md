...
- `--stock-hot-floor <pct>` / `--stock-hot-reload <pct>` — sous `--stock-max-ram`, le stock froid est rangé dans un **étage RAM en blocs** (≈ 70 octets par possibilité contre 112 en liste chaînée, avec ou sans disque) ; la liste chaude y descend au-dessus de son plancher (défaut 25 % du plafond) et en remonte sous son seuil de rechargement (défaut 10 %). Sauvegardé dans le `.back`, remplacé par `restore`, lu par l'expansion. Voir [Utilisation](docs/utilisation.md#étage-ram-en-blocs---stock-hot-floor---stock-hot-reload).
