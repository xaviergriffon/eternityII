...
- `--local-dispatch` — client : le process parent relaie au serveur les délégations de ses forks, au lieu que chaque fork paie un aller-retour TCP synchrone **par possibilité** depuis son thread de recherche. Désactivée par défaut. Voir [Utilisation](docs/utilisation.md#relais-des-délégations-par-le-parent---local-dispatch).
