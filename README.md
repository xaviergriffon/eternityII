...
- `--sort-enabled` [`--sort-interval <n>`] [`--sort-direction asc|desc`] — tri périodique du stock par file (sans regroupement, comme `sortAscFiles`/`sortDescFiles`), désactivé par défaut ; suspendu tant qu'un client est connecté, même garde-fou que `removeNoNext`. Voir [Utilisation](docs/utilisation.md#tri-périodique-du-stock---sort-enabled).
