...
- `--no-rmnonext` — ne démarre PAS l'élagage automatique des possibilités sans suite, actif par défaut (seule option-drapeau négative du programme). Sur une pile très longue, une passe de `removeNoNext` parcourt tout le stock en tenant les files et peut saturer le serveur ; la commande console `removeNoNext` reste disponible pour un élagage manuel, au moment choisi. Voir [Utilisation](docs/utilisation.md#élagage-automatique-des-possibilités-sans-suite---no-rmnonext).
