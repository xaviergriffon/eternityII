...
etii_cl_instance *create_etii_cl_instance(cl_device_type device_type, map_big_array *map, int max_research)
{
	...
	cl_program program;
	program = CL_CHECK_ERR(clCreateProgramWithSource(context, 1, (const char **)&source_str, (const size_t *)&source_size, &_err));
	...
	cl_mem map_buffer;
	map_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(struct array_part) * mapinone->nbarrays, NULL, &_err));
	cl_mem parts_buffer;
	parts_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(struct part) * mapinone->nbparts, NULL, &_err));
	cl_mem key_buffer;
	key_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(key_part)*max_research, NULL, &_err));
	cl_mem faceused_buffer;
	faceused_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(uint8_t)*256*max_research, NULL, &_err));
    
    cl_mem output_buffer;
	output_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(struct part)*max_research*RESULT_BY_SEARCH, NULL, &_err));
	cl_mem qt_buffer;
	qt_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int)*max_research, NULL, &_err));
	...
	idesc.num_samples = 0;
	idesc.buffer = NULL;
    cl_mem img_map = CL_CHECK_ERR(clCreateImage(context, CL_MEM_READ_ONLY, &iformat_map, &idesc, NULL, &_err));
    ...
    cl_mem img_datas = CL_CHECK_ERR(clCreateImage(context, CL_MEM_READ_ONLY, &iformat_datas, &idesc_datas, NULL, &_err));
...
//			print_part(&part);
	...
	for(int i=0; i<mapinone->nbparts;i++)
	{
		CL_CHECK(clEnqueueWriteBuffer(queue, parts_buffer, CL_TRUE, i*sizeof(struct part), sizeof(struct part), &mapinone->parts[i], 0, NULL, NULL));
	}
    
    free_map_in_one(mapinone);
...
File **test_opencl(etii_cl_instance *instance, key_part *keys, struct possibility_packet *possiblity, int nbresearch)
{
	...
	for(r=0; r< nbresearch; r++)
	{
		...
		init_file_with_cache(results[r], 300, sizeof(struct part));
	}
...
//	printf("size of part : %lu\n",sizeof(struct part));
	...
	/*
    cl_kernel kernel;
	kernel = CL_CHECK_ERR(clCreateKernel(instance->program, "search_part", &_err));
	CL_CHECK(clSetKernelArg(kernel, 0, sizeof(instance->key_buffer), &instance->key_buffer));
	...
	CL_CHECK(clSetKernelArg(kernel, 5, sizeof(instance->map_buffer), &instance->map_buffer));
	CL_CHECK(clSetKernelArg(kernel, 6, sizeof(int), &instance->mapsizearray));
    */
	
	cl_kernel kernel;
	kernel = CL_CHECK_ERR(clCreateKernel(instance->program, "search_part_img", &_err));
	CL_CHECK(clSetKernelArg(kernel, 0, sizeof(instance->key_buffer), &instance->key_buffer));
	CL_CHECK(clSetKernelArg(kernel, 1, sizeof(instance->output_buffer), &instance->output_buffer));
	CL_CHECK(clSetKernelArg(kernel, 2, sizeof(instance->qt_buffer), &instance->qt_buffer));
    int result_by_search = RESULT_BY_SEARCH;
...
int free_etii_cl_instance(etii_cl_instance *instance)
{
	...
	CL_CHECK(clReleaseMemObject(instance->parts_buffer));
