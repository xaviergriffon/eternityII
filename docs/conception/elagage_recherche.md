...
**Instrumentation — écrite, utilisée, puis RETIRÉE** (même discipline que §4.8 pour
`ETII_ARENA_ORDER` : une fois la mesure faite et consignée, garder un instrument que plus
aucun chemin n'exécute n'a pas de valeur). Le protocole est décrit ici assez précisément
pour être réécrit en une heure si la forme de l'arbre change un jour ; le résultat, lui, ne
dépend plus de sa présence.

À chaque nœud du moteur MRV, pour chacune des 4 zones d'angle : l'anneau des 6 cases
extérieures est-il entièrement posé alors que la zone est encore trouée ? Deux masques de
bits constants par angle (zone, anneau), et le test tient en deux `AND` sur
`bt_frontier.empty` — déjà tenu en lockstep avec le plateau, donc rien à reconstruire. Le
tout derrière un `#ifdef` : la boucle chaude est limitée par le débit d'émission (IPC ≈
3,15, [autosearch_step.md](../autosearch_step.md)), une instrumentation inconditionnelle
perturberait le débit qu'elle mesure. Trois grandeurs relevées en plus du simple compteur,
parce que le compteur seul ne suffit pas à conclure :
...
d'arbre — le compteur reste à **zéro sur 1,17 milliard de nœuds**. (Cette mesure-là exige
le mode `client`, qui refusait alors de démarrer ses forks sous `ETII_BENCH_NODES` : un
point de rendez-vous `fork_gate` manquant, corrigé au passage — voir
[tests_et_ci.md](../tests_et_ci.md#banc-de-mesure-du-débit-de-recherche-testsbenchbench_searchsh).) Les racines réelles sont
...
- **L'instrumentation n'est pas conservée** — même arbitrage qu'en §4.8 : la mesure est
  faite et consignée, le protocole ci-dessus suffit à la refaire. Ne pas retoucher
  `directions[]` pour provoquer artificiellement la situation : bump de `VERSION` (§5).
