...
Deux drapeaux existaient au moment de l'écriture de ce document, tous deux à 0 par défaut et
**délibérément indépendants** : `mrv_enabled` (`ETII_MRV=1`, la recherche) et `pruner_dfs_mrv`
(`ETII_PRUNER_DFS_MRV=1`, la preuve du pruner). La proposition était de les supprimer
tous les deux en même temps que le moteur à ordre fixe — fait en PR3 (§8) : les deux
drapeaux et le moteur à ordre fixe n'existent plus dans `master`.
...
| PR | Contenu | Critère de succès | Statut |
|---|---|---|---|
