...
**Bilan cumulé des points 8 et 9** (E5-2630 v4, 3 M nœuds, `perf stat` en rondes alternées contre le binaire de master, σ < 0,3 %) :
...
| Compteurs `fc_*` incrémentés **sans** `lock` (build de mesure, pas thread-safe) | **×1,048** — c'est la borne haute du cumul local de la PR #260 (fermée sans fusion, +1,00 % à l'époque) : le poids relatif des 4,6 `lock add` par nœud a quadruplé avec l'allègement du balayage. À re-décider, pas fait ici. |
