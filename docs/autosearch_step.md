...
5. **Départage des égalités par les côtés contraints.** À score MRV égal — cas fréquent, le minimum valant 1 dans plus de la moitié des appels — la case dont le PLUS de côtés sont déjà contraints (`bt_frontier.nconstr` : bords de grille + voisines posées) l'emporte, au lieu de la première rencontrée. Le critère est gratuit à lire, `nconstr` étant maintenu de toute façon par la frontière. **Son sens a été mesuré, pas déduit** : l'heuristique de degré classique de la littérature CSP recommande l'inverse (`nconstr` minimal, soit le plus de voisines encore vides) et se révèle nettement perdante ici. Sur la population entière des racines fermables d'un stock de production (433 racines, 383 fermées par les deux) : **−6,29 % de nœuds, −5,01 % de temps, et 4 sous-arbres fermés de plus** à budget égal ; la variante classique coûte +4,4 %. Depuis §4.16 ce critère n'est plus le second mais le TROISIÈME : le **poids d'échec appris** est passé au-dessus de lui (point 11), et `nconstr` ne départage plus que les égalités de score dom/wdeg, avant l'ordre d'énumération. Sa direction, elle, n'a pas changé. Protocole et pièges : [elagage_recherche.md §4.12](conception/elagage_recherche.md).

6. **Cache du pointeur de masque par case** (`cell_mask`, `bt_mask_init`/`bt_mask_refresh`). Les points 1 à 3 avaient rendu chaque case de frontière bon marché, mais il en restait 54,5 par nœud et chacune payait encore un `map_bucket_id_mask` complet : recalcul de l'index 4D (3 `imul` + 3 `add`), lecture de `packed[idx]` — un accès **dispersé dans 1,3 Mio** — puis dérivation de l'adresse du masque. Or la clé d'une case ne change QUE lorsqu'une voisine est posée ou retirée : `bt_propagate_place`/`_undo` n'écrivent que les ≤4 voisines. Le pointeur est donc mémorisé par case et rafraîchi aux mêmes endroits, ce qui ramène le balayage de 54,5 résolutions par nœud à **~13**. Mesuré E5-2640 v4, 3 M nœuds × 5 répétitions alternées sous `perf stat` : **1,185 → 1,447 M nœuds/s, ×1,22** pour le cache seul, **5 097 → 4 441 instructions par nœud (−12,9 %)**, IPC 2,57 → 2,72, `max_result` (190) et taux d'élagage inchangés. C'est le **quatrième** cache en lockstep avec le plateau, et `bt_mask_refresh` s'appelle À CÔTÉ de `bt_propagate_place`/`_undo` comme `bt_frontier_place`/`_undo` — un site de placement peut donc l'oublier. Deux garde-fous : la re-dérivation des builds `DEBUG_CHECK_POSSIBILITY`, et `bt_mask_cache_stays_in_lockstep_with_constraints` (`tests/core/test_etii_search.c`), qui porte un **contre-contrôle** — une pose sans rafraîchissement doit faire diverger le cache, sans quoi le test ne prouverait plus rien si `bt_mask_refresh` devenait un no-op.

7. **Réduction du minimum sans branchement, par clé composite.** Le point 6 a eu un effet de bord que seul `perf` a révélé : en supprimant ~656 instructions par nœud, il a **triplé les mauvaises prédictions de branchement** du balayage (7,0 → 12,8 par nœud), qui rendaient ~90 cycles par nœud. Le profil `perf record -e branch-misses` les localise sur les comparaisons du choix lui-même (`count < best_count`, puis le départage `nconstr`) : des tests portant sur des données sans structure, donc structurellement imprévisibles, et que gcc compilait en sauts plutôt qu'en `cmov`. Les trois critères sont désormais empilés dans **une seule valeur entière** — `key = (count << 3) | (7 - nconstr)` — dont l'ordre naturel EST la règle de choix : `count` croissant en poids fort, `nconstr` décroissant en poids faible, et le `<` strict qui laisse la main au premier rencontré à égalité complète. La réduction se ramène alors à une comparaison et deux affectations conditionnelles. **Le sens du départage reste donc encodé, pas perdu** : remplacer `(7 - nconstr)` par `nconstr` fait échouer exactement les deux tests annoncés (`mrv_choose_cell_matches_full_scan_reference` et `mrv_choose_cell_breaks_ties_by_constrained_sides`) — contre-contrôle effectué. La largeur du champ bas est verrouillée à la compilation (`MRV_KEY_NC_BITS`, `mrv_key_nc_fits_check`) : si `nconstr` débordait sur `count`, c'est le critère MRV principal qui serait faussé, silencieusement. Mesuré : **mauvaises prédictions 12,8 → 6,6 par nœud, +201 instructions par nœud** (les deux branches sont désormais toujours exécutées) mais **−8,1 % de cycles**, soit **1,476 → 1,606 M nœuds/s, ×1,088**. C'est le cas d'école où *ajouter* des instructions accélère.

8. **Chemin rapide « masques complets »** (`mrv_choose_cell_fast`, `bt_forward_check_fast`, `bt_masks_complete`). Re-profilé après le point 7 (E5-2630 v4, `perf stat` + `perf record` mappé sur le désassemblage, 3 M nœuds) : **67 % des instructions et 53 % des cycles** d'un nœud restaient dans le balayage, soit **~57 instructions par case de frontière** — dont **~35 d'échafaudage de généricité** : le calcul de `&constraints[x][y]` (servi au seul repli `mask == NULL`, jamais pris en production), le test NULL par case, le **rechargement de `map->id_mask_words` à chaque case** (charge conditionnelle que gcc ne peut pas hisser) et la **cascade `cmp/je` d'une boucle de mots à borne dynamique**. Les mauvaises prédictions, elles, se concentraient sur deux sauts : la sortie de la boucle `while (bits != 0)` du balayage (37,6 %) et le saut d'un candidat déjà utilisé (17,4 %, dépend des données), puis les sorties de la boucle de mots de `map_mask_any_free` dans le forward-check (~10 %). Or trois faits sont des **propriétés de la map, constantes pour tout le processus** : l'index compact et `bucket_id_mask` existent, `id_mask_words == (ETERN_PARTS + 63) / 64` (4 pour 256, 1 pour 16), et le seul NULL possible dans `cell_mask` est le compartiment vide. Ils sont donc décidés **une fois par recherche** (`bt_masks_complete`) et non par case : en mode rapide, `bt_mask_init`/`bt_mask_refresh` substituent au NULL d'un compartiment vide un **masque à zéro partagé** (`bt_zero_mask`, même verdict que le parcours de 0 entrée — le contrat de `map_bucket_id_mask` ne bouge pas, la substitution vit dans le cache du moteur), et les deux fonctions `_fast` sont compilées avec `BT_MASK_WORDS` **constant**, sans test NULL, sans adresse de clé, avec `~used` calculé une fois par appel. Les versions génériques restent telles quelles pour les maps bâties à la main (tests) ou hors gabarit, et le choix est un branchement par nœud parfaitement prédit. Mesuré : **instructions −29 %, branchements −54 %, mauvaises prédictions −45 %, cycles −17 %, ×1,21** à arbre identique. Verrouillé par `mrv_choose_cell_fast_matches_generic_on_real_map` (chemin rapide = version générique = oracle indépendant, 300 plateaux, id maximal forcé à `ETERN_PARTS` pour que la map soit « complète » quelle que soit la taille compilée), `bt_forward_check_fast_same_verdict_as_generic` et `bt_mask_cache_uses_zero_mask_for_empty_bucket_in_fast_mode` ; la suite `ETERN_PARTS=16` exerce le chemin rapide de bout en bout sur la vraie map 4×4.

9. **Chaque µop par case coûte ~0,25 cycle × 54 cases.** Une fois le point 8 en place, le balayage tourne à **IPC ≈ 3,15, c'est-à-dire au débit d'émission** : ni la mémoire ni les ports ne bornent (mesuré : aucun port au-delà de 53 %, `resource_stalls.rs` 13 % des cycles). Le seul levier restant est le nombre de µops par case, et les deux réductions retenues le confirment : (a) le bas de la valeur de choix — `((7 − nconstr) << 8) | pos` — est **précalculé par la frontière** (`bt_frontier.nc_key`, tenu en lockstep avec `nconstr` aux trois mêmes sites d'écriture), si bien que clé, départage et position forment **une seule valeur** dont le minimum strict est exactement la règle MRV (positions énumérées en ordre croissant : à clé égale la plus petite position gagne, donc la première rencontrée) — une charge, un `or`, un `cmp`, un `cmov` par case au lieu de sept instructions : **×1,040** ; (b) la détection de case morte quitte la boucle (**×1,010**, −4,8 % d'instructions) : une case sans candidat porte `count = 0` en poids fort, elle est forcément le minimum, et le verdict se lit après la réduction — les nœuds morts sont trop rares pour que le balayage complet qu'ils paient alors compte. **Deux hypothèses « mémoire » ont été réfutées par la mesure sur cette même boucle** (tableau ci-dessous) : copier les masques en ligne par case pour supprimer l'indirection `cell_mask[pos]` ne gagne rien, et sauver/restaurer les masques des voisines au retrait (au lieu de les re-résoudre) perd — 128 o copiés par pose coûtent plus que 4 résolutions. La règle du §1.3 ter tient toujours, sous une autre forme : **compter les µops, pas les cycles de latence.**

10. **Les compteurs de prunage étaient un verrou pris contre personne** (`fc_stat_bump`). `fc_attempts`, `fc_pruned`, `fc_cells_studied` et `fc_pruned_at[]` étaient incrémentés par `__atomic_fetch_add` — **~4,6 lectures-modifications-écritures verrouillées par nœud**, chacune sérialisant le pipeline. Or, dans un processus donné, **un seul thread les écrit** : `run_mono_client` (`app/etii_client.c`) exécute `autosearch`/`autoprune` dans le thread courant, et `NB_THREADS` compte des FORKS — des processus, chacun avec sa propre copie des globales, agrégées ensuite par IPC (`fork_statistics`). Les autres threads du processus (alimentation, contrôle, statistiques) ne font que LIRE. Une lecture relâchée suivie d'une écriture relâchée produit donc exactement le même résultat pour un écrivain unique, en `mov`/`add`/`mov` sans préfixe `lock`. **Ce n'est pas une course de données** : les deux accès restent atomiques au sens du modèle mémoire C11, seule l'atomicité de l'ENSEMBLE lecture+écriture disparaît — celle dont un écrivain unique n'a pas besoin. C'est déjà, mot pour mot, le contrat de `counters[]`, le compteur de nœuds : écrit en clair par ce même thread, lu par le thread de statistiques. Les chemins FROIDS gardent `__atomic_fetch_add` (`bt_materialize_pending`, et `possibility.c` que le thread de console peut atteindre via `removeNoNext`) : ils n'ont rien à y gagner. Mesuré : **−5,02 % de cycles, 8/8 rondes positives**.

    **Pourquoi pas le cumul local publié périodiquement** (PR #260, fermée sans fusion en août 2026, +1,00 % à l'époque) : rebasée sur ce chemin rapide elle vaut désormais +4,44 %, mais elle coûte une **fraîcheur** que ce projet mesure. `bench_poll_and_maybe_stop` (`app/etii_client.c`) lit `fc_attempts`/`fc_pruned` **pendant** que la recherche tourne ; une fenêtre en attente de publication met en défaut l'identité `fc_attempts = nœuds + fc_pruned`, exacte à 1 près par construction (la racine n'est précédée d'aucune tentative). Mesuré sur 8 exécutions :

    | écart à l'identité | min | médiane | max |
    |---|---|---|---|
    | master (atomiques) | 0 | 1 | 1 |
    | cumul local, fenêtre 512 | 48 | **199** | 392 |
    | écriture non verrouillée | 1 | **1** | 1 |

    L'écriture non verrouillée gagne donc **plus** (−5,02 % contre −4,44 %) en ne perdant **aucune** précision, et ne demande ni constante de fenêtre à régler, ni publication sur chaque chemin de sortie, ni retouche des tests existants. Le taux d'élagage rapporté, lui, ne distingue plus les trois variantes (biais −0,00007 point pour une étendue naturelle de 0,00134 point entre deux exécutions du même binaire) : **c'est l'identité, pas le taux, qui rend la staleness visible** — à retenir si la question se repose.

11. **Poids d'échec appris, en critère PRINCIPAL** (`bt_frontier.weight`, `bt_frontier_fail`, `mrv_wdeg_shl`, §4.14 puis §4.16 de [elagage_recherche.md](conception/elagage_recherche.md)). Chaque refus du forward-check incrémente un compteur saturant sur la voisine morte ET sur la case posée (`bt_fc_failed`) ; à saturation (255) tous les poids sont divisés par deux, et ils repartent de zéro à chaque racine (l'apprentissage est intra-racine). §4.14 en avait fait un départage de troisième rang, sous `nconstr` : −79,6 % de nœuds de réfutation. §4.16 en fait le **critère principal**, à la `dom/wdeg` (Boussemart et al., 2004) — le poids DIVISE le score MRV et prime donc sur `nconstr` : `score = count << mrv_wdeg_shl(poids)`, où le décalage descend de 4 à 0, un de moins par DOUBLEMENT du poids (plafond ×16). **Un décalage, pas une division** : la boucle est bornée par l'émission (point 9), une division y serait rédhibitoire et une table de réciproques coûterait un `imul` de plus ; `wshl` est un octet par case, tenu en lockstep avec le poids. La clé composite RÉTRÉCIT au passage — le poids quitte les champs bas, `nc_key` passe de 32 à 16 bits et vaut `(7 − nconstr) << 8 | pos` — et le score gagné ne contenant plus `count`, celui-ci est recompté pour la SEULE case gagnante (quatre `popcount` par nœud, hors boucle chaude). **L'invariant de détection de case morte est intact** : un décalage est une multiplication par une puissance de deux, donc `score = 0 ⟺ count = 0`, et `(best >> MRV_KEY_LOW_BITS) == 0` reste exactement le verdict de la production. Mesuré sur trois régimes de profondeur disjoints d'un stock de production, plafond 2 M de nœuds : **−96 % de nœuds appariés et 497 → 500 fermetures** sur les racines ≥ 130 pièces, **124 → 287 fermetures sur 300** entre 100 et 120 pièces (le régime que §4.14 déclarait non concluant), point de chute en baisse de 4,5 à 25 points, bout à bout **16,3 s → 0,40 s**. Coût propre du mécanisme, isolé sur un arbre identique au nœud près : **+1,4 %**, sous le plancher de bruit. Le poids n'est pas un état dérivable du plateau : la re-dérivation `DEBUG_CHECK_POSSIBILITY` ne le compare pas.

**Bilan cumulé des points 8 et 9** (E5-2630 v4, 3 M nœuds, `perf stat` en rondes alternées contre le binaire de master, σ < 0,3 %) — le point 10, mesuré séparément contre le master qui les contient déjà, y ajoute **×1,053** :

| par nœud | avant (0ca64a4) | après | |
|---|---|---|---|
| cycles | 1 487 | 1 176 | **−20,9 %** |
| instructions | 4 646 | 2 931 | **−36,9 %** |
| mauvaises prédictions | 6,67 | 3,53 | −47,1 % |
| IPC | 3,12 | 2,49 | −20 % (attendu : le travail retiré était le plus parallèle) |
| débit | 1,60 M nœuds/s | **2,03 M nœuds/s** | **×1,26** |

`fc_attempts`, `fc_pruned` et `max_result` (190) identiques à chaque ronde : c'est du coût retiré, pas un arbre différent.

**Ce qui a été mesuré et ÉCARTÉ sur le chemin rapide** (E5-2630 v4, même protocole, 3 rondes alternées chacune) :

| Piste | Résultat |
|---|---|
| Masques copiés **en ligne** par case (`uint64_t[256][4]`, plus d'indirection `cell_mask[pos]`) | **×0,99** — la boucle n'est pas bornée par la latence de l'indirection |
| Sauvegarde des masques des voisines à la pose, **restauration** au retrait (au lieu de re-résoudre) | **×0,97** avec les masques en ligne (128 o copiés par pose) ; avec le cache de pointeurs le retrait ne coûte que 4 résolutions, rien à gagner |
| `mrv_choose_cell_fast` hors inlining (`noinline`, pour lui rendre ses registres) | **×1,00** — les rechargements de `~used` depuis la pile ne coûtent rien |
| Indexation par base de mot (`cell_mask + w*64`, sans recomposer `pos`) | **×1,00** — gcc le faisait déjà ; gardé pour la lisibilité |
| Cumul local des compteurs `fc_*` publié tous les 512 nœuds (PR #260 rebasée) | **×1,046**, mais met en défaut l'identité `fc_attempts = nœuds + fc_pruned` (écart médian 199 contre 1) — remplacé par l'écriture non verrouillée du point 10, qui gagne plus sans rien coûter |

**Bilan cumulé des points 6 et 7** (E5-2640 v4, `performance`, 3 M nœuds × 9 répétitions alternées sous `perf stat`, σ = 3 ms) :

| par nœud | avant (dbcc260) | après | |
|---|---|---|---|
| cycles | 1 994,6 | 1 474,1 | **−26,1 %** |
| instructions | 5 114,7 | 4 639,5 | −9,3 % |
| branchements | 880,9 | 707,5 | −19,7 % |
| mauvaises prédictions | 7,03 | 6,62 | −5,8 % |
| IPC | 2,564 | **3,147** | +22,7 % |
| débit | 1,189 M nœuds/s | **1,609 M nœuds/s** | **×1,353** |

Décomposition : cache de masque **×1,220**, grandes pages **×1,016** de plus, clé sans branchement **×1,088** de plus.
...
| Grandes pages 2 Mio (`alloc_hugepage_zeroed`, `src/core/part.c`) | ×1,045 **seul**, mais **×1,016 seulement** une fois le point 6 en place — retenu quand même, sans risque de correction |

L'échec de NUMA et des grandes pages a la même cause, déjà établie par l'encadré de §1.3 ter : le jeu de travail chaud ne quitte jamais L1/L2, la DRAM n'est jamais sur le chemin critique, et les défauts de dTLB sont absorbés par le STLB puis recouverts par l'exécution dans le désordre. Les compteurs le chiffrent **directement**, et c'est la mesure la plus contre-intuitive de la série : `dtlb_load_misses.stlb_hit` tombe de **63,4 à 0,36 par nœud** avec les grandes pages (÷176, la couverture TLB passant de 4 Kio à 2 Mio par entrée) — pour **1,6 % de gain**. Un succès de STLB coûte donc ici ~0,4 cycle effectif, pas les ~7 cycles de sa latence nominale : le hors-ordre le recouvre presque entièrement. **Tout raisonnement TLB fondé sur le majorant latence × nombre surestime d'un facteur ~17 sur cette boucle** — c'est un compteur à lire, jamais à extrapoler.
