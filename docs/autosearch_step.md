...
**Publication différée des compteurs.** `bt_forward_check` n'écrit plus les globales `fc_*` : il cumule dans un `fc_local_stats` fourni par son appelant, et c'est la boucle de backtracking qui publie — tous les `FC_STATS_PUBLISH_INTERVAL_NODES` nœuds, et à chaque sortie (fin de sous-arbre, arrêt, budget, et avant `record_solution` qui peut ne pas revenir). Une lecture-modification-écriture atomique par compteur touché et par tentative de placement était du coût pur pour une donnée lue une fois par seconde : ~4,6 atomiques par nœud ramenées à ~0,023, **+1,00 % de débit** (A/B apparié à ordre alterné, 6 rondes × 7 répétitions, 6/6 positives, p = 0,0156). Les totaux restent exacts et monotones — publier ajoute, n'écrase pas — mais un lecteur qui échantillonne *pendant* la recherche peut manquer une fenêtre : voir [tests_et_ci.md](tests_et_ci.md) pour ce que ça implique sur la précision du taux d'élagage rapporté par le banc, et pourquoi c'est cette contrainte-là, et non l'amortissement, qui fixe la taille de la fenêtre.
...
**Quatre choses rendent le choix abordable**, là où le prototype de mesure de la PR 9 tombait à 23 k nœuds/s (−99,7 %) :

1. **Balayage restreint à la frontière.** Une case dont les 4 côtés valent `all_face` n'est contrainte par rien (ni bord de plateau, ni voisine posée) : elle accepte toutes les pièces libres et ne peut donc jamais être le minimum tant qu'une case contrainte existe. La frontière compte 29 cases en moyenne (max 52) contre 256 cases balayées par le prototype.
