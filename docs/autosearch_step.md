...
11. **Départage appris par poids d'échec** (`bt_frontier.weight`, `bt_frontier_fail`, §4.14 de [elagage_recherche.md](conception/elagage_recherche.md)). À score MRV et `nconstr` égaux, l'ordre positionnel `pos` tranchait — un critère qui ignore la recherche en cours. Désormais chaque échec du forward-check incrémente un compteur saturant sur la voisine morte ET sur la case posée (`bt_fc_failed`), et la case au poids le plus ÉLEVÉ passe d'abord ; à saturation (255) tous les poids sont divisés par deux, et les poids repartent de zéro à chaque racine (l'apprentissage est intra-racine, mesuré à moins d'un point d'un apprentissage persistant). La clé composite s'élargit d'un champ : `count << 19 | (7 − nconstr) << 16 | (255 − poids) << 8 | pos`, `nc_key` passe à 32 bits et reste précalculé par la frontière — le balayage garde une charge et un `or` par case. Mesuré sur les 418 racines ≥ 130 pièces d'un stock de production, plafond 2 M de nœuds : **410 fermetures contre 378, −79,6 % de nœuds sur les racines fermées par les deux**, ratio médian 0,045 sur les racines ≥ 10 000 nœuds ; le sens inverse coûte +210 %, et un tirage aléatoire des égalités bat déjà l'ordre positionnel de 37 à 56 %. Débit par nœud inchangé. Le poids n'est pas un état dérivable du plateau : la re-dérivation `DEBUG_CHECK_POSSIBILITY` ne le compare pas.
