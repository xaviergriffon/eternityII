...
| `statistic` | Affiche la répartition du stock par nombre de pièces posées (`alloc`, voir [le champ `alloc`](api_http_rest.md#le-champ-alloc)), pools non vérifié et vérifié confondus, avec la difficulté moyenne (`min_candidats`, voir [le champ `min_candidats`](api_http_rest.md#le-champ-min_candidats)) quand elle est mesurée pour ce niveau, puis deux lignes de débit ADD/GET (événements `put_to_pool`/`scroll_from_pool`, tous pools confondus) moyennées sur la dernière minute, la dernière heure et le dernier jour (alias : `stats`). Non exécutable à distance (ni canal de contrôle, ni `POST /api/v1/command`) puisqu'elle n'écrit que dans les journaux ; l'équivalent exploitable par une application tierce est [`GET /api/v1/stock-distribution`](api_http_rest.md#get-apiv1stock-distribution) pour la répartition par `alloc`, et [`GET /api/v1/stats`](api_http_rest.md#get-apiv1stats) (champs `stock_adds_per_sec_*`/`stock_removes_per_sec_*`) pour le débit ADD/GET |
