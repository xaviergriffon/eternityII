...
`sortaf` (sortAscFiles), `sortd` (sortDesc), `sortdf` (sortDescFiles), `sortdm` (sortDescMulti), `rmnonext` et `prune` (removeNoNext),
...
| `config` | **Client/pruner**, sans argument : affiche l'état de l'orchestrateur de démarrage différé (`WAITING_CONFIG`/`COUNTDOWN`/`CONFIGURING`/`RUNNING`/…, avec le temps restant avant auto-démarrage en `COUNTDOWN`), la configuration **effective** (`nb_forks`, `server_host`, `parts_file`, `max_stock_by_thread`, `shallow_root_abandon_depth`, `limit`, `pruner_batch`, `dfs_budget` — reflète les globales courantes, y compris un `limit`/`maxStockByThread`/`shallowRootAbandonDepth`/`prunerBatch`/`prunerDfsBudget` déjà exécuté depuis la console) et la configuration **en préparation**. N'annule pas le décompte. **Serveur**, sans argument : affiche la configuration **effective** du serveur (`nb_threads`, `parts_file`, `expand_level`, `expand_max_stock`, `expand_max_levels`, `http_port`, `http_token_file`, `stock_files`, `stock_max_ram`, `stock_spill_dir`, `rebalance_budget`, `tcp_timeout`, `auto_roles`, `stop_on_solution`, `headless`) et le fichier `--config-file` en vigueur — pas d'état d'orchestrateur ni de configuration "en préparation", le serveur n'en a pas |
...
| `shallowRootAbandonDepth N` | Ajuste à la volée la profondeur d'abandon d'une racine reçue trop peu profonde (0 = désactivé) — voir [docs/utilisation.md](utilisation.md#option---shallow-root-abandon-depth-client-et-pruner) |
| `prunerBatch N` | Ajuste la taille de lot d'échange du pruner à la volée (borné à [1, 65536]) |
| `prunerDfsBudget N` | Ajuste à la volée le budget de nœuds de la preuve de fermeture bornée du pruner (§4.6b) : une possibilité jugée vivante par le contrôle superficiel mais pas encore `checked` est rejouée par un backtracking réel plafonné à `N` nœuds — si ce budget suffit à épuiser tout son sous-arbre, elle est prouvée morte (aucun faux positif) et jamais redistribuée, sinon comportement inchangé (conservée, `checked`). `N <= 0` désactive ce contrôle supplémentaire (comme `limit 0`) ; borné à [0, 10000000] ; **activé par défaut à 10000** (§4.6c) — mesuré en conditions réelles : sur un stock de production remis au pool non vérifié, un pruner à 3 forks élimine 22 006 possibilités sur 32 480 (67,8 %) en moins de 5 minutes, palier reproduit à l'identique sur une seconde machine. La valeur est un arbitrage et non un optimum (la courbe budget/fermeture ne plafonne pas : 56,4 % à 1000, 66,4 % à 10000, 73,2 % à 100000) ; `1000` reste un choix conservateur. **Ce réglage ne touche que le flux** : le passif déjà `checked` ne repasse devant la preuve qu'après un `resetChecked` (ci-dessous), à lancer une fois après déploiement sur un serveur dont le stock est déjà constitué |
...
| `sortDescFiles` | Trie chaque file par ordre décroissant, **sans les regrouper** : sur toutes les files (pas seulement la première), consomme en priorité les possibilités les moins avancées (alias : `sortdf`) |
...
| `removeNoNext` | Supprime les possibilités sans continuation possible (élagage ; alias : `rmnonext`, `prune`). Pendant MANUEL du thread d'élagage automatique du serveur — seul recours quand celui-ci est désactivé par [`--no-rmnonext`](utilisation.md#élagage-automatique-des-possibilités-sans-suite---no-rmnonext---rmnonext-interval) |
...
| `rebalance [n]` | Rééquilibre le stock d'un seul pas incrémental (file la plus pleine → la plus vide, `n` possibilités par pool, défaut `rebalance_budget` réglable via `--rebalance-budget`) — le même appel que celui automatique de chaque tour serveur (10 s), déclenché immédiatement ; contrairement à `split`, ne redistribue pas intégralement en un appel. Pendant MANUEL de ce tour automatique — seul recours quand celui-ci est désactivé par [`--no-rebalance`](utilisation.md#rééquilibrage-automatique-du-stock---no-rebalance) |
| `resetChecked` | Bascule l'**intégralité** du pool vérifié vers le pool non vérifié, `checked` remis à 0 pour chaque possibilité déplacée — elle redevient donc servable via `INST_GET_TO_CHECK`, comme n'importe quelle possibilité jamais encore vérifiée (aucun câblage de distribution supplémentaire : c'est déjà le chemin existant). Escape hatch pour `autoprune_step` (`src/core/etii_search.c`), qui ne retente **jamais** la preuve de fermeture bornée (`search_packet_backtracking_budgeted`) sur une possibilité déjà `checked == 1` : sans cette commande, un réglage ultérieur de `prunerDfsBudget` (ou toute autre amélioration du prunage) ne profite jamais au passif déjà vérifié. Déplacement **en place** (même index de file, jamais round-robiné ailleurs). Le pool « en cours d'analyse » n'est pas concerné, même précédent que `checkOrigin`. Coût trivial par entrée (contrairement aux O(n²)/O(n log n) de `checkOrigin`/`checkDuplicate`) : balaie tout le pool vérifié en un seul appel, pas de découpage incrémental façon `rebalance` |
| `stockMemory` | Affiche le [plafond RAM du stock](utilisation.md#plafond-ram-du-stock---stock-max-ram) et l'occupation actuelle (Mo, possibilités) — deux pools comptés ensemble, jamais le pool analysé — la part rangée dans l'[étage RAM en blocs](utilisation.md#étage-ram-en-blocs---stock-hot-floor---stock-hot-reload) (compression active, possibilités, Mo, octets par possibilité, et taille de la liste chaude), le [débordement sur disque](utilisation.md#débordement-sur-disque-du-stock---stock-spill-dir) (toujours affichés, même à 0) et le total liste + étage + déporté |
...
| Commande | Description |
|---|---|
| `backup` | Pendant MANUEL de la sauvegarde automatique périodique — seul recours quand celle-ci est désactivée par [`--no-autobackup`](utilisation.md#sauvegarde-automatique-périodique---no-autobackup). Sauvegarde les files de possibilités dans `eternityII.back` et `eternityII-in_analyse.back` (stock et pool analysé capturés à un instant T unique, gelés puis libérés progressivement file par file), ainsi que le meilleur plateau connu (`eternityII-best_board.back`), le cumul par machine (`eternityII-known_clients.back`, voir [Registre de clients connus](echanges_client_serveur.md#registre-de-clients-connus)) — chaque fichier n'est réécrit que si son propre contenu a changé depuis la dernière écriture. Sauvegarde **autonome** : si [`--stock-spill-dir`](utilisation.md#débordement-sur-disque-du-stock---stock-spill-dir) est actif, le débordement disque est recopié DANS `eternityII.back` (après la libération des files, jamais sous le gel), si bien que ce seul fichier se restaure sur une autre machine ou sous un plafond RAM plus bas ; une recopie incomplète fait échouer la commande sans toucher à la sauvegarde précédente. **Échoue** (sans rien écrire du stock) pendant une passe d'expansion, où une partie du stock est hors des pools — à relancer ensuite, voir [Utilisation](utilisation.md#sauvegarde-automatique-périodique---no-autobackup) |
| `restore [fichier [fichier_analyse]]` | Restaure les files depuis les fichiers `.back` (remplace le stock) ; recharge aussi, sans argument dédié, le meilleur plateau connu et le cumul par machine (absence tolérée). Débordement disque : un `.back` **autonome** (produit par `backup` ou l'arrêt sur solution) le porte déjà — le débordement courant est vidé et aucun cliché n'est consulté ; sinon (`temp.back` de l'autobackup, sauvegarde antérieure), le cliché que nomme `<fichier>.spillcount` (`snapshot-temp/`, `snapshot/` à défaut) est remis en place. Si ce cliché ne correspond pas à ce qui a été sauvegardé — `--stock-spill-dir` oublié/différent, cliché supprimé — la commande **échoue explicitement** plutôt que de le taire, voir [Utilisation](utilisation.md#débordement-sur-disque-du-stock---stock-spill-dir). **Refusée** pendant une expansion (`--expand-level`, `expand`) : la passe en cours réinjecterait l'ancien stock dans le stock restauré |
...
| Commande | Description |
|---|---|
| `check` | Affiche le dernier rapport de statistiques (n'efface plus l'écran — utiliser `clear`). Côté client/pruner, contient le tableau par fork, la ligne forward-check et la ligne `pruner : …` (voir `statistic` ci-dessous, même ligne, même source) |
...
| `statistic` | Affiche la répartition du stock par nombre de pièces posées (`alloc`, voir [le champ `alloc`](api_http_rest.md#le-champ-alloc)), pools non vérifié et vérifié confondus, avec la difficulté moyenne (`min_candidats`, voir [le champ `min_candidats`](api_http_rest.md#le-champ-min_candidats)) quand elle est mesurée pour ce niveau, puis deux lignes de cumul ADD/GET (nombre d'événements `put_to_pool`/`scroll_from_pool`, tous pools confondus) sur la dernière minute, la dernière heure et le dernier jour glissants, ventilées par pool (vérifié/non vérifié) sur les deux lignes suivantes, puis, **sur un process client ou pruner ayant contrôlé au moins une possibilité**, la ligne `pruner : <mortes> mortes / <contrôlées> vérifiées (<taux>), <n> cases étudiées` — agrégat des compteurs remontés par les forks (`pruner_checked`/`pruner_removed`/`pruner_cells_studied`, cf. [Communication parent ↔ enfants](architecture.md#communication-parent--enfants-ipc)), exactement la même ligne que celle du rapport `check` et construite par la même fonction ; absente tant qu'aucun prunage n'a eu lieu, donc jamais affichée côté serveur ni sur un client de recherche. Puis les métriques de besoin par rôle : compteurs de service à vide (`K = 0`) par rôle depuis le démarrage, et le parc actuellement connecté ventilé recherche/contrôle (alias : `stats`). Non exécutable à distance (ni canal de contrôle, ni `POST /api/v1/command`) puisqu'elle n'écrit que dans les journaux ; l'équivalent exploitable par une application tierce est [`GET /api/v1/stock-distribution`](api_http_rest.md#get-apiv1stock-distribution) pour la répartition par `alloc`, et [`GET /api/v1/stats`](api_http_rest.md#get-apiv1stats) (champs `stock_adds_last_*`/`stock_removes_last_*`, leurs variantes `*_checked_*`/`*_unchecked_*`, `server_search_starved`/`server_prune_starved`, `nb_search_sessions`/`nb_prune_sessions`) pour l'équivalent complet |
| `checkDatas` | Vérifie l'intégrité des possibilités |
| `checkDuplicate` | Recherche les doublons exacts (`compare_possibility`) et les relations ancêtre/descendant (`is_origin_of`) dans le stock, par comparaison exhaustive multi-thread (8 threads, O(n²) paires). Balaie les **deux** pools (non vérifié et vérifié), adressés comme une seule séquence virtuelle de `2 * nb_file_possibility` files (`duplicate_pool_at`, `core/datamanager.c`) — un doublon scindé entre les deux pools, ou logé entièrement dans le pool vérifié, est détecté au même titre qu'un doublon interne au pool non vérifié (avant ce correctif, seul le pool non vérifié était balayé). Rapporte chaque erreur dans les journaux (comptée, jamais supprimée — contrairement à `checkOrigin purge`, aucun mode de purge n'existe ici) et rend `-1` s'il en existe au moins une |
| `checkFiles` | Vérifie l'intégrité de toutes les files |
| `checkOrigin [purge]` | Vérifie qu'aucune possibilité en stock n'est la **racine** d'une autre — une possibilité dont toutes les cases posées se retrouvent à l'identique dans une autre, plus profonde (`is_origin_of`), rend celle-ci redondante : son sous-arbre est déjà couvert. Balaie les **deux** pools (non vérifié et vérifié), rapporte chaque relation trouvée (100 lignes de détail au plus, puis le seul total) et rend `-1` s'il en existe au moins une. Avec `purge` : supprime chaque **descendant** et garde la **racine**, jamais l'inverse — le sous-arbre du descendant est inclus dans celui de sa racine, donc aucune branche de recherche n'est perdue, et une seconde passe ne trouve alors plus rien. La raison de garder la racine plutôt que le descendant, plus avancé : on ne sait pas ce qui a produit ces paires, donc l'état intermédiaire n'est pas digne de confiance et le travail est à refaire depuis la racine — quitte à jeter le prunage déjà payé d'un descendant `checked` (arbitrage tranché, voir [conception/checkorigin_stock_deborde.md](conception/checkorigin_stock_deborde.md)). Lancer `backup` ensuite pour graver l'état purgé (la purge ne déclenche pas de sauvegarde automatique). Sans argument, le stock n'est jamais modifié ; tout autre argument que `purge` est refusé. Coût en **O(n²)** sur la taille du stock (le nombre de paires à comparer est annoncé au démarrage) : commande de diagnostic, pas de routine. Ne balaie que le stock **résident** — ce qui a débordé sur disque (voir [`spill`](utilisation.md#plafond-ram-du-stock---stock-max-ram)) est signalé mais pas comparé, et le pool « en cours d'analyse » est hors périmètre. Détecte aussi les **doublons exacts** — deux possibilités de même `alloc` et de contenu strictement identique (`compare_possibility`), une bande que le tri par profondeur fait sauter à la détection racine/descendant ci-dessus (à `alloc` égal, ni l'une ni l'autre n'est la racine de l'autre). Trouvaille en production : sur une sauvegarde de 10 855 possibilités, 0 relation racine/descendant mais une paire de doublons stricts en `alloc=112`, tous deux `checked`, invisible à `checkOrigin` avant ce correctif — et à `checkDuplicate` avant le sien (celui-ci ne balayait alors que le pool non vérifié ; corrigé depuis, voir la ligne `checkDuplicate` ci-dessus). Rapportée séparément (« doublon exact » vs « racine de », même plafond de 100 lignes) mais pliée dans le même total et le même code de retour `-1`. Avec `purge`, un groupe de N doublons identiques (N ≥ 2, pas seulement les paires) ne garde qu'**un seul survivant**, déterministe (le premier du groupe dans l'ordre trié par `alloc`) — les autres sont supprimés, qu'ils soient doublons, descendants, ou les deux à la fois (chaque entrée n'est de toute façon supprimée qu'une fois) |
...
| `élagage automatique activé (intervalle Ns)` | Côté serveur, au démarrage, quand l'élagage automatique est lancé — `N` vaut `--rmnonext-interval` (défaut 30) |
| `élagage automatique désactivé (--no-rmnonext) : …` | Côté serveur, au démarrage, quand l'élagage automatique n'est pas lancé (`--no-rmnonext` ou `rmnonext_enabled = 0`, voir [Utilisation](utilisation.md#élagage-automatique-des-possibilités-sans-suite---no-rmnonext---rmnonext-interval)) — le stock qui ne décroît plus de lui-même est alors voulu ; `removeNoNext` reste disponible à la demande |
| `sauvegarde automatique désactivée (--no-autobackup) : …` | Côté serveur, au démarrage, quand la sauvegarde périodique n'a plus lieu (`--no-autobackup` ou `autobackup_enabled = 0`, voir [Utilisation](utilisation.md#sauvegarde-automatique-périodique---no-autobackup)) — un `temp.back` qui ne vieillit plus est alors voulu ; `backup` reste disponible à la demande, et reste le SEUL filet en cas d'arrêt brutal. Aucune ligne symétrique dans le cas actif : le diagnostic de démarrage porte déjà `autobackup_enabled=oui` |
| `rééquilibrage automatique du stock désactivé (--no-rebalance) : …` | Côté serveur, au démarrage, quand le rééquilibrage incrémental de chaque tour n'a pas lieu (`--no-rebalance` ou `rebalance_enabled = 0`, voir [Utilisation](utilisation.md#rééquilibrage-automatique-du-stock---no-rebalance)) — des files qui dérivent en taille sont alors voulues ; `rebalance [n]` reste disponible à la demande. Aucune ligne symétrique dans le cas actif : le diagnostic de démarrage porte déjà `rebalance_enabled=oui` |
