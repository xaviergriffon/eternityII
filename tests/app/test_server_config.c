...
/* autobackup_enabled : troisième clé booléenne de défaut 1, même domaine 0/1 —
   c'est le 0 qui est la demande explicite (--no-autobackup). */
TEST parse_line_autobackup_enabled_accepts_zero_and_one(void)
{
    server_config_t cfg;
    server_config_init(&cfg);
    ASSERT_EQ_FMT(SERVER_CONFIG_LINE_SET, server_config_parse_line("autobackup_enabled = 0\n", &cfg), "%d");
    ASSERT_EQ_FMT(1, cfg.has_autobackup_enabled, "%d");
    ASSERT_EQ_FMT(0, cfg.autobackup_enabled, "%d");
    ASSERT_EQ_FMT(SERVER_CONFIG_LINE_SET, server_config_parse_line("autobackup_enabled = 1\n", &cfg), "%d");
    ASSERT_EQ_FMT(1, cfg.autobackup_enabled, "%d");
    PASS();
}

TEST parse_line_autobackup_enabled_rejects_other_values(void)
{
    server_config_t cfg;
    server_config_init(&cfg);
    ASSERT_EQ_FMT(SERVER_CONFIG_LINE_INVALID_VALUE, server_config_parse_line("autobackup_enabled = 2\n", &cfg), "%d");
    ASSERT_EQ_FMT(SERVER_CONFIG_LINE_INVALID_VALUE, server_config_parse_line("autobackup_enabled = non\n", &cfg), "%d");
    PASS();
}
...
/* Même piège que les deux autres clés de défaut 1 : autobackup_enabled = 0 est
   une clé PRÉSENTE, que le format doit écrire -- sinon un configSave perdrait
   la désactivation et le redémarrage suivant sauvegarderait de nouveau tout
   seul. */
TEST format_includes_autobackup_enabled_even_when_zero(void)
{
    server_config_t cfg;
    server_config_init(&cfg);
    cfg.has_autobackup_enabled = 1;
    cfg.autobackup_enabled = 0;

    char buf[256];
    int n = server_config_format(&cfg, buf, sizeof(buf));
    ASSERT(n > 0);
    ASSERT(strstr(buf, "autobackup_enabled = 0") != NULL);
    PASS();
}

/* La configuration EFFECTIVE complète (toutes les clés présentes, c'est ce que
   `config`/`configSave` formatent) doit tenir dans le tampon de 1024 octets que
   ces deux appelants déclarent -- sinon server_config_format rend -1 et
   configSave refuse d'écrire. Chaque clé ajoutée rogne cette marge : ce test
   est le garde-fou qui le signale au lieu de le découvrir en production. */
TEST format_of_the_effective_config_fits_the_caller_buffer(void)
{
    server_config_t cfg;
    server_config_capture_effective(&cfg);

    char buf[1024]; /* même taille que dans ui/command_lines.c et server_config_save */
    int n = server_config_format(&cfg, buf, sizeof(buf));
    server_config_free(&cfg);

    ASSERT(n > 0);               /* -1 = tronqué */
...
/* autobackup_enabled : le fichier peut couper la sauvegarde automatique quand
   la CLI ne l'a pas fait (globale encore à son défaut 1). */
TEST apply_pre_dispatch_autobackup_disabled_by_file_when_global_is_default(void)
{
...
/* Priorité CLI > fichier, dans le sens propre à cette clé : --no-autobackup a
   déjà mis la globale à 0, un autobackup_enabled = 1 du fichier ne doit PAS
   rallumer la sauvegarde automatique. */
TEST apply_pre_dispatch_autobackup_keeps_cli_disable_over_file_enable(void)
{
...
/* La configuration effective rapporte toujours la clé (présente, 0 ou 1) :
   c'est ce que `config` affiche et ce que `configSave` persiste. */
TEST capture_effective_reports_autobackup_enabled(void)
{
    server_autobackup_enabled = 0;
...
SUITE(server_config_suite)
{
    ...
    RUN_TEST(parse_line_autobackup_enabled_accepts_zero_and_one);
    RUN_TEST(parse_line_autobackup_enabled_rejects_other_values);
    RUN_TEST(format_includes_autobackup_enabled_even_when_zero);
    RUN_TEST(format_of_the_effective_config_fits_the_caller_buffer);
    RUN_TEST(apply_pre_dispatch_autobackup_disabled_by_file_when_global_is_default);
    RUN_TEST(apply_pre_dispatch_autobackup_keeps_cli_disable_over_file_enable);
    RUN_TEST(capture_effective_reports_autobackup_enabled);
